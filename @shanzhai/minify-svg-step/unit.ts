  ...
  describe(`on iterating`, () => {
    ...
    describe(`with invalid input`, () => {
      ...
      beforeAll(async () => {
        ...
        try {
          await minifySvgStep.iterate(original);
          error = null;
        } catch (e) {
          error = e;
        }
      ...
      it(`throws the expected error`, () => {
        expect(error).toEqual(
          new Error(`Failed to optimize SVG: SvgoParserError: <input>:1:496: Unclosed root tag

> 1 | …16" y="16" width="224" height="224" fill="#fff"/></svg
    |                                                       ^
.`)
        );
      });
